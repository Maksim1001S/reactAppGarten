...
const ProductsContainer = ({ products_data, page_container }) => {
  ...
  const FilteredAndSortedProducts = () => {
    const filteredProducts = products_data.filter((el) => {
      ...
      return (
        (sort.discounted ? el.discont_price !== undefined : true) &&
        (sort.price.priceFrom ? actualPrice >= sort.price.priceFrom : true) &&
        (sort.price.priceTo ? actualPrice <= sort.price.priceTo : true)
      );
    });
    ...
    switch (sort.sortedBy) {
      ...
      case "byDesc":
        return filteredProducts.sort((a, b) => {
          ...
          return actualPrice_a - actualPrice_b;
        });
      case "byAsc":
        return filteredProducts.sort((a, b) => {
          const actualPrice_a = a.discont_price ? a.discont_price : a.price;
          const actualPrice_b = b.discont_price ? b.discont_price : b.price;
          return actualPrice_b - actualPrice_a;
        });
      default:
        return filteredProducts;
    }
  ...
  return (
    <div className={s.products_wrapper}>
      ...
      {visibleProducts < filteredProducts.length ? (
        <div className={s.load_more_btn} onClick={handleLoadMore}>
          Load More
        </div>
      ) : (
        ""
